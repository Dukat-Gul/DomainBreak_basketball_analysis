...
class TacticalViewConverter:
    def __init__(self, court_image_path):
        ...
        self.basket_3d_coordinates = np.array(
            [
                [self.actual_width_in_meters / 2, 0, 3.05],  # Basket center
            ]
        )

    def transform_3d_to_2d(self, point_3d, frame_keypoints):
        """
        Transform a 3D point to a 2D point in the video frame using homography.

        Args:
            point_3d (tuple): The (x, y, z) coordinates of the 3D point in real-world units (meters).
            frame_keypoints (list): List of detected court keypoints in the video frame.

        Returns:
            tuple: The (x, y) coordinates of the projected 2D point in the video frame, or None if
                the homography cannot be computed.
        """
        ...
        # For simplicity, we are using x and z from the 3d coordinates and transforming them
        point_2d_for_transform = np.array([[point_3d[0], point_3d[2]]])
        transformed_point = homography.transform_points(point_2d_for_transform)
